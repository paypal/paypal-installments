...
export function initiateInstallments({
  clientID,
  Installments,
  paymentMethodID,
  button,
  ...
  cartAmount,
  onPay,
  getLogger = getFlowLogger,
}: installmentsProps): Function {
  const inputs = {
  ...
  return getInstallments(inputs)
    .then((installmentsResponse) => {
      if (
        installmentsResponse &&
        installmentsResponse.getInstallmentsForVaultedToken
      ) {
        ...
        const showInstallmentsModal =
          ...
          (installmentsData[0] && installmentsData[0].discount);
        ...
        if (showInstallmentsModal) {
          ...
          const data = {
            ...
            onPay: (selectedInstallment) => {
              ...
              getLogger()
                .info(
                  `installments_pay_button_clicked_${
                    selectedInstallment ? selectedInstallment.term : ""
                  }x`
                )
                .track({
