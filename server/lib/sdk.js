...
export function sdkMiddleware(
  { logger = defaultLogger, cache }: SDKMiddlewareOptions,
  {
    app,
    script,
    preflight,
    ...
    app: SDKMiddleware,
    script?: SDKScriptMiddleware,
    preflight?: SDKPreflightMiddleware,
  |}
): ExpressMiddleware {
    ...
    try {
      ...
      try {
        ...
        return clientErrorResponse(
          res,
          `Invalid params: ${safeJSON(req.query)}`
        );
      }
      ...
      try {
        ...
        return clientErrorResponse(
          res,
          `Invalid sdk meta: ${(req.query.sdkMeta || "").toString()}`
        );
      }
      ...
      if (isError(err, ERROR_CODE.VALIDATION_ERROR)) {
        logger.warn(req, EVENT.VALIDATION, {
          err: err.stack ? err.stack : err.toString(),
        });
  ...
  const preflightMiddleware = async (
    req: ExpressRequest,
    res: ExpressResponse
  ): Promise<void> => {
    logBuffer.flush(req);
  ...
  const middleware = async (
    ...
    if (url === "/script") {
      res.header(
        ...
        `public, max-age=${BROWSER_CACHE_TIME}`
      );
      res.header(
        HTTP_HEADER.EXPIRES,
        // eslint-disable-next-line no-mixed-operators
        new Date(Date.now() + BROWSER_CACHE_TIME * 1000).toUTCString()
      );
