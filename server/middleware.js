...
export function getInstallmentsMiddleware({
  logger = defaultLogger,
  content: smartContent,
  cache,
}: InstallmentsMiddlewareOptions = {}): ExpressMiddleware {
  return sdkMiddleware(
    { logger, cache },
    {
      app: async ({ req, res, params, meta, logBuffer }) => {
        ...
        const { clientID, cspNonce, debug, locale } = getParams(
          params,
          req,
          res
        );

        const client = await getInstallmentsClientScript({
          debug,
          logBuffer,
          cache,
        });
        ...
        if (!clientID) {
          return clientErrorResponse(
            res,
            "Please provide a clientID query parameter"
          );
        }
        ...
        const pageHTML = `
                <!DOCTYPE html>
                <head></head>
                <body data-nonce="${cspNonce}" data-client-version="${
          client.version
        }">
                    ${meta.getSDKLoader({ nonce: cspNonce })}
                    <script nonce="${cspNonce}">${client.script}</script>
                    <script nonce="${cspNonce}">installmentsModal.setupInstallments(${safeJSON(
          { cspNonce, content }
        )})</script>
                </body>
            `;
